...
int main() {
    int tempoGastoEmHoras, velMediaKmh, distanciaPercorrida;
    float calculoEmLitros;

    printf("Digite o tempo gasto durante a viagem: ");
    scanf("%d", &tempoGastoEmHoras);
    printf("\n");
    printf("Digite a velocidade média (KM/H): ");
    scanf("%d", &velMediaKmh);

    distanciaPercorrida = velMediaKmh * tempoGastoEmHoras;

    calculoEmLitros = (float)distanciaPercorrida / 12;
